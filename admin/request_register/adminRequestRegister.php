...
<div class="container-fluid bg-warning text-center text-dark p-2 fs-5">
        ...
        <span class="badge bg-danger"><?= isset($_SESSION["req_num"]) ? $_SESSION["req_num"] : "0" ?></span>
</div>
<!-- notice section -->
    ...
    <div class="container mt-3 container-style" >
        ...
        <?php foreach($req_data as $item): ?>
            ...
            <?php if($item->reject !=1 ): ?>
                ...
                <!-- reject တွင် 1 ဆိုရင် reject  -->
                <!-- reject တွင် 2 ဆိုရင် reject မဟုတ်ဘူး -->

                <?php if(trim($item->ad_veri_code) == "not yet"):?>
                    <div class="create-style-div">
                        <div class="create-main">
                            
                            <?php if($item->show == 0 || $item->show == 2) : ?>
                        ...
                        <a href="../../_action/class_join_data/register_delete.php?id=<?= $item->id?>&&rd=<?= $random?>&&d=arr" class="btn btn-danger" onclick="return confirm('Are you Sure?')">Delete</a>
                            <a href="adminRequestDetail.php?id=<?= $item->id?>&&rd=<?= $random?>" class="btn btn-secondary">Detail</a>
                        </div>
